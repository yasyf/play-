...
if ($player == 1) { //side-scrolling player
    ...
    $sql = "UPDATE `yasyfcom_play`.`" . $game . "` SET player1x=" . $player1x . " WHERE turn=" . $turn;
    mysql_query($sql);
    ...
    $sql = "UPDATE `yasyfcom_play`.`" . $game . "` SET box1x=" . $box1x . ",box2x=" . $box2x . ",box3x=" . $box3x . " WHERE turn=" . $turn;
    mysql_query($sql);
}
?>