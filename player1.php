<?php
	...
	$sql = "SELECT box1x,box2x,box3x,box1y,box2y,box3y,player2ready
			 FROM `yasyfcom_play`.`".$game."`
			WHERE turn=".$turn;
			$result = mysql_query($sql);
				while($data = mysql_fetch_row($result)){
				   echo("box1x=$data[0]&box2x=$data[1]&box3x=$data[2]&box1y=$data[3]&box2y=$data[4]&box3y=$data[5]&player2ready=$data[6]");
				}
?>